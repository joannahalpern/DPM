...
private static final long CORRECTION_PERIOD = 10; //How frequently the correction iterates in ms
private Odometer odometer;
private static int lightVal = 0; // the value read by the light sensor
private static final int LIGHT_THRESHOLD = 350; // if lightVal is lower than this number, we know there is a dark surface
// these values determine the origin on the physical grid. This sets the origin to the center
//of the first square, but these variables change
// in our more general code (commented out throughout this class), which sets the origin to the starting point of the robot
...
if (lightVal < LIGHT_THRESHOLD) { // if it sees a line (because then the lightVal will decrease below LIGHT_THRESHOLD)
...
//inversePosition maps the grid with the origin at gridOffset(x, y) to a grid with the origin
//at the bottom left of the physical tile the robot was placed in. This is done because the next method, nearest30() requires
//a grid with the origin at the bottom left corner
y = inversePositionY(y, theta);

// nearest30 calculates the nearest grid line it is at so that it can set the coordinate to that value
y = nearest30(y);

//This maps the origin of the grid back to the center of the starting tile
...
/**
* This method compensates for the fact that the center of the first tile
* is considered (0,0) by subtracting gridOffset(x, y) from the initial (x,y).
...
* This is needed because the method nearest30() only works when the origin is the physical bottom left corner of the starting tile.
* (15.24,15.24) is the origin we are given
*/
